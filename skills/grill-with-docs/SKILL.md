...
3. Inspect the repository before asking questions its source, tests, configuration, schemas, or existing decisions can answer. Use `repository-discovery` for a material or unfamiliar codebase.
...
10. End with a coverage audit and hand the stable discovery record to `implementation-plan`.
...
Return the task path, glossary changes, ADRs created or proposed, resolved decisions, remaining blockers, and a recommendation on readiness for `implementation-plan`. Stop without coding.
