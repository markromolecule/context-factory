# Claude Entry Point & Dispatch Contract
...
Before executing any non-trivial user request, Claude MUST resolve the exact required skills, rules, and workflows:
...
- Do not output hidden chain-of-thought or xml wrapper tags unless requested.
- Verify every claim by inspecting repository files before making statements.
- Run `node scripts/context.mjs lock` and `node scripts/context.mjs doctor` after context updates.
