...
Before executing any non-trivial user request, the assistant MUST resolve the exact required skills, rules, and workflows:
...
- Adhere strictly to the vertical backend module boundaries and frontend styling guidelines.
- Never write or edit production code when fulfilling plan-only requests.
- Always run `node scripts/context.mjs lock` and `node scripts/context.mjs doctor` after modifying context rules, skills, or templates.
