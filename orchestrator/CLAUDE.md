# Claude & Claude Code Entry Point Contract
...
- MUST: Avoid generic LLM frontend styling by adhering to `rules/ui/frontend.md` and prototyping with `skills/playground/SKILL.md`.
...
| `tsc`, `compiler error`, `type error`, `circular types` | [`skills/typescript/SKILL.md`](skills/typescript/SKILL.md) | Diagnose and resolve compiler errors and type failures. |
...
| `zod`, `schema validation`, `runtime validation`, `dto parsing` | [`skills/zod/SKILL.md`](skills/zod/SKILL.md) | Model runtime schemas, DTOs, and boundary validation contracts. |
| `api contract`, `openapi`, `scalar`, `client sdk` | [`skills/api-contract/SKILL.md`](skills/api-contract/SKILL.md) | Model type-safe endpoints, OpenAPI/Scalar specs, and client SDKs. |
| `database query`, `query optimization`, `index tuning`, `cursor pagination` | [`skills/database-query/SKILL.md`](skills/database-query/SKILL.md) | Optimize SQL queries, execution plans, and cursor pagination. |
| `component craft`, `react component`, `accessible ui`, `cls prevention` | [`skills/component-craft/SKILL.md`](skills/component-craft/SKILL.md) | Author polished, accessible, responsive React/Next.js components. |
| `test suite`, `4-layer testing`, `mock boundaries`, `hook smoke test` | [`skills/test-suite/SKILL.md`](skills/test-suite/SKILL.md) | Construct 4-layer backend tests and client hook smoke tests. |
