...
- For a new system, product, or materially ambiguous feature, use `grill` as the first pre-planning skill. Resolve and persist goals, scenarios, language, boundaries, and unknowns before `plan`; do not begin production coding until the plan is approved.

## Roles & Subagents

Software delivery lifecycles are orchestrated across specialized subagent personas defined under `agents/`:

- **BA Agent (`agents/ba-agent` / Discovery lead):** clarify ambiguous requirements, resolve unknowns one question at a time using `grill`, and establish verifiable scenario matrices and acceptance criteria before planning.
